...
//-----------------------------------------------------------------------------------
SOCKET UDPSocket::CreateUDPSocket(char const *addr,
    ...
    sockaddr_in *out_addr) // address we actually connected to.
{
    ...
    addrinfo *info_list = NetSystem::AllocAddressesForHost(addr, // an address for this machine
...
void UDPSocket::Bind(const char* address, const char* portNumber)
{
    ...
    while (m_socket == INVALID_SOCKET && ++currNumRetries < MAX_RETRIES)
    {
        ...
        unsigned int port = atoi(portNumber) + 1;
        _itoa_s(port, portStr, bufferSize, 10);
        m_socket = CreateUDPSocket(address, portStr, &m_address);
        delete portStr;
    }
    if (currNumRetries == MAX_RETRIES)
    {
        ERROR_RECOVERABLE(Stringf("Was unable to bind to a port after %i retries", MAX_RETRIES));
    }
}
